...
pos = find(error_val == min(error_val));
lambda = lambda_vec(pos);
theta = trainLinearReg(X_poly, y, lambda);
error_test = linearRegCostFunction(X_poly_test, ytest, theta, 0);
fprintf('Err at lbd=%d on Xtest = [%f]\n', lambda, error_test);

%% =========== Part 10: Optional :: Small training size =====================
...
for i = 1:m
   ...
   for j = 1:cnt
      rand = randperm(m); 
      pos = rand(1:i);
      theta = trainLinearReg(X_poly(pos, :), y(pos, :), lambda);      
      cost = cost + linearRegCostFunction(X_poly_val, yval, theta, 0);
   end
   error_test(i, 1) = cost / cnt;
end
error_test
